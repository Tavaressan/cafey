"""Validacao do arranjo interno (variante 3D, arranjo_3d.FCStd).

Confere as faixas, a altura livre, a separacao rede/baixa tensao e conflitos
grosseiros entre os volumes de referencia contra a nova geometria impressa
(paredes de parede=2.4 mm, sem abas de canto). Todos os tamanhos de
componentes.csv sao ESTIMADOS - preservados 1:1 da versao em chapa (issue
#116 pede manter o arranjo interno).

Rode build_peca1_3d.py, build_peca2_3d.py e build_arranjo_3d.py antes.
Uso: freecadcmd mechanical/scripts/check_arranjo_3d.py
"""
...
def main():
    ...
    for nome, c in comps.items():
        ...
        if c["apoio"] != "parede":
            # dentro das paredes em planta (a parede agora e' mais grossa:
            # 2.4 mm contra 1.2 mm da chapa)
            check(b.XMin >= g["parede"] - 0.05 and b.XMax <= g["pegada_x"] - g["parede"] + 0.05
                  and b.YMin >= g["parede"] - 0.05 and b.YMax <= g["pegada_y"] - g["parede"] + 0.05,
                  "%s: dentro das paredes em planta" % nome)
            # nao penetra a Peca 1 (paredes / flange / tampo)
            inter = o.Shape.common(peca1).Volume
            check(inter < 1.0, "%s: sem penetrar a peca impressa (%.2f mm3)" % (nome, inter))
    ...
    bd = dv.BoundBox
    check(bd.XLength >= g["pegada_x"] - 2 * g["parede"] - 1,
    ...
    check(e.YMin <= g["parede"] + 8,
          "ESP32 com a antena junto a' parede frontal (Ymin %.1f)" % e.YMin)
